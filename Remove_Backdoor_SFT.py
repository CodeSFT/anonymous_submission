...
def main(parser, transform_train, transform_test):
    ...
    cl_test_loss, ACC =SFT_Test(model=net, criterion=criterion, data_loader=clean_test_loader)
    po_test_loss, ASR =SFT_Test(model=net, criterion=criterion, data_loader=poison_test_loader)
    ...
    for i in range(nb_iterations):
        lr = args.lr
        train_loss, train_acc = SFT_Train(args,i, net, clean_val, clean_val_loader, criterion_reg)

        clean_loss , ACC = SFT_Test(model=net, criterion=criterion, data_loader=clean_test_loader)
        poison_loss, ASR = SFT_Test(model=net, criterion=criterion, data_loader=poison_test_loader)
...
## Training Scheme
def SFT_Train(args,epoch, net, clean_val, clean_val_loader, criterion_reg):
    print('\nEpoch: %d' % epoch)
    net.train()
    train_loss = 0
    correct = 0
...
def SFT_Test(model, criterion, data_loader):
    model.eval()
    total_correct = 0
    total_loss = 0.0
    with torch.no_grad():
